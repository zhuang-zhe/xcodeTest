...
- (void)renderBox:(easyar_Matrix44F *)projectionMatrix cameraview:(easyar_Matrix44F *)cameraview box:(NSArray<easyar_Vec3F *> *)box {
    glBindBuffer(GL_ARRAY_BUFFER, vbo_coord_box);
    GLfloat cube_vertices[8][3];
    for (int i = 0; i < 8; ++i)
        for (int j = 0; j < 3; ++j)
            cube_vertices[i][j] = [[[box objectAtIndex:i].data objectAtIndex:j] floatValue];
    glBufferData(GL_ARRAY_BUFFER, sizeof(cube_vertices), cube_vertices, GL_DYNAMIC_DRAW);
    ...
    glVertexAttribPointer(pos_color_box, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, 0);
    for (int i = 0; i < 6; i++) {
        glDrawElements(GL_LINE_STRIP, 4, GL_UNSIGNED_SHORT, (void*)(i * 4 * sizeof(GLushort)));
    }
}
